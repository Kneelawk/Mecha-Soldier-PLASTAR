...
public class ModelConverter {
     ...
     */
    public static Model build(GltfModel model, Map<String, ResourceLocation> textureMap) {
        ...
        for (var nodeModel : model.getNodeModels()) {
            ...
            var transform = new Matrix4f(FloatBuffer.wrap(transformValues));
            ...
            for (var meshModel : meshModels) {
                for (var primitiveModel : meshModel.getMeshPrimitiveModels()) {
                    var material = SimpleMaterial.builder()
                        ...
                        .mipmap(false)
                        .texture(textureMap.get(primitiveModel.getMaterialModel().getName()))
                        .build();
